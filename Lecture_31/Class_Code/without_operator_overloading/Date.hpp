...
class Date
{
 private:
    ...
    bool IsDateValid(unsigned short day, MONTH month, unsigned long year)
    {
 ...
 public:
    ...
    {
      m_day.setDay(m_day.getDay() + 1);

      if(!IsDateValid())
      {
        if(m_month.GetMonth() == MONTH::December)
          m_year.SetYear(m_year.GetYear() + 1);

        m_month.SetMonth(m_month.IncrementDateMonth());
    ...
    void DecrementDateDay()
    {
      ...
      if(!IsDateValid())
      {
        if(m_month.GetMonth() == MONTH::January)
          m_year.SetYear(m_year.GetYear() - 1);

        m_month.SetMonth(m_month.DecrementDateMonth());

        m_day.setDay(m_month.GetLastDay(m_year.IsLeapYear()));
      }
    }
